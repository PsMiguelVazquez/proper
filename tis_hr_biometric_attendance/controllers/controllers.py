...
class ZKTecoController(http.Controller):
    ...
    @http.route(['/iclock/cdata','/iclock/cdata.aspx'], type='http', auth='public', methods=['GET', 'POST'])
    ...
    @http.route(['/iclock/registry', '/iclock/registry.aspx'],
                type='http', auth='public', methods=['POST'], csrf=False)
    def handle_registry(self, **kwargs):
        serial_number = kwargs.get('SN')
        _logger.info(f"Device registry request received from SN: {serial_number}")

        registry_code = datetime.now().strftime("%m%d%H%M%S")

        if serial_number:
            device_id = request.env['biometric.config'].sudo().search([('serial_number', '=', serial_number)], limit=1)
            if device_id:
                device_id.sudo().write({'last_update': fields.Datetime.now(), 'state': 'connected'})
                _logger.info(f"Device {serial_number} connected. RegistryCode={registry_code}")
            else:
                _logger.warning(f"Unknown device tried to register: {serial_number}")
        else:
            _logger.warning("Registry request received without SN parameter.")

        return Response(f"RegistryCode={registry_code}", status=200)

    @http.route(['/iclock/push', '/iclock/push.aspx', '/iclock/ping', '/iclock/querydata'],
                type='http', auth='public', methods=['POST'], csrf=False)
    ...
    def device_command(self, **kwargs):
        ...
        for line in raw_data.split('\n'):
            ...
            if (param_dict.get("CMD") == "DATA" or param_dict.get("CMD") == "CHECK"
                ...
                device_id.check_user_command_response(command_id, param_dict)
        return Response("OK", 200)
