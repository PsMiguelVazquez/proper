...
class BiometricDeviceConfig(models.Model):
    ...
    attendance_data_log_ids = fields.One2many(
        'attendance.data.log.device',
        'device_id',
        string="Attendance Data Logs"
    )
 ...
 # Adjust if necessary (e.g., if device name is used for logs)
    ...
    def action_open_face(self):
        self.ensure_one()
        return {
            'type':'ir.actions.act_window',
            'name':'User Face template',
            'view_mode':'list,form',
    ...
    def create_device_user(self, values):
        ...
        # Extract card number from Card=[...] format
        cardno = None
        for value in values:
            if value.startswith('Card='):
                # Extract the card number from Card=[value] format
                card_match = re.search(r'Card=\[([^\]]*)\]', value)
                if card_match:
                    cardno = card_match.group(1)
                break
        ...
        for line in raw_data.splitlines():
            ...
            if self.used_for == "both":
                if self.is_status_number_based:
                    ...
                    last_log_ids = self.env['attendance.log'].search([('device_user_id', '=', db_user_device_id.id)]).sorted('id')
                    if last_log_ids:
    ...
    def create_finger_print(self, values):
        device_user_id = values[1].split('=', 1)[1]
        f_id = values[2].split('=', 1)[1]
        f_size = values[3].split('=', 1)[1]

        # Store TMP exactly as received (even if it has "=" or "==")
        fp_temp = values[5].split('=', 1)[1]
        ...
        if not db_user_device_id:
            db_user_device_id = self.env['biometric.attendance.devices'].sudo().create({
                'biometric_attendance_id': device_user_id,
                'device_id': self.id
            })

        fp_exist = self.env['finger.template'].sudo().search([
            ('device_id', '=', self.id),
            ('device_user_id', '=', db_user_device_id.id),
            ('f_id', '=', f_id)
        ])
        ...
        if fp_exist:
            fp_exist.template_data = fp_temp
            fp_exist.f_id = f_id
            fp_exist.f_size = f_size
        else:
            self.env['finger.template'].sudo().create({
                'device_user_id': db_user_device_id.id,
                'device_id': self.id,
                'template_data': fp_temp,
                'f_id': f_id,
                'f_size': f_size
            })
    ...
    def create_face(self, values):

        device_user_id = values[2].split('=')[1]  # PIN
        f_id = values[3].split('=')[1]  # FID
        f_size = values[4].split('=')[1]  # Size
        fp_temp = values[6].split('=')[1]  # TMP (face template from device)

        # Do not change template, store as-is
        tmp = fp_temp

        # Find or create the biometric user record
        db_user_device_id = self.env['biometric.attendance.devices'].sudo().search([
            ('device_id', '=', self.id),
            ('biometric_attendance_id', '=', device_user_id)
        ], limit=1)
        ...
        # Check if face template already exists for same FID
        face_exist = self.env['face.template'].sudo().search([
            ('device_id', '=', self.id),
            ('device_user_id', '=', db_user_device_id.id),
            ('f_id', '=', f_id)
        ], limit=1)
        ...
        if face_exist:
            ...
            self.env['face.template'].sudo().create({
                'device_user_id': db_user_device_id.id,
                'device_id': self.id,
                'template_data': tmp,
                'f_id': f_id,
    ...
    def check_user_command_response(self, command_id, response_dict=None):
        if command_id:
            command_exist = self.env['device.command'].search([('id', '=', command_id)])
            if command_exist:

                # If device ACK returned (Return=0), mark success immediately
                if response_dict and response_dict.get("Return") == "0":
                ...
                # DATA UPDATE USERINFO
                elif command_exist.employee_id and command_exist.name == "DATA UPDATE USERINFO":
                    device_user_id = self.env['biometric.attendance.devices'].sudo().search([
                        ('biometric_attendance_id', '=', command_exist.pin),
                        ('device_id', '=', self.id)
                    ])
                    ...
                    if device_user_id:
                        device_user_id.write({
                            'employee_id': command_exist.employee_id.id,
                            'device_user_name': command_exist.employee_id.name
                        })
                    else:
                        device_user_id = self.env['biometric.attendance.devices'].sudo().create({
                            'biometric_attendance_id': command_exist.pin,
                ...
                elif command_exist.employee_id and command_exist.name == "DATA UPDATE FINGERTMP":
                    values = command_exist.execution_log.split()
                    pin = values[2].split('=', 1)[1]  # PIN
                    fid = values[3].split('=', 1)[1]  # FID
                    size = values[4].split('=', 1)[1]  # Size
                    tmp = values[7].split('=', 1)[1] if len(values) > 7 else ""
                    ...
                    if device_user_id:
                        finger = self.env['finger.template'].sudo().search([
                ...
                elif command_exist.employee_id and command_exist.name == "ENROLL_FACE":
                    # Example command_text:
                    # C:45:ENROLL_BIO TYPE=2 PIN=1234  FID=0  RETRY=1  OVERWRITE=1
                    values = command_exist.execution_log.split()
                    pin = values[2].split('=', 1)[1]  # TYPE=2 (skip, just confirm it's face)
                    pin = values[3].split('=', 1)[1]  # PIN
                    fid = values[4].split('=', 1)[1]  # FID
                    ...
                    if device_user_id:
                        face = self.env['face.template'].sudo().search([
                            ('device_user_id', '=', device_user_id.id),
                            ('device_id', '=', self.id),
                            ('f_id', '=', fid)
                        ])
                        if not face:
                            self.env['face.template'].sudo().create({
                                ...
                                'employee_id': command_exist.employee_id.id,
                                'f_id': fid,
                                'template_data': '',  # actual template data may come later from device
                            })
                        else:
                            # Already exists, just update employee
                            face.employee_id = command_exist.employee_id.id

                    command_exist.status = 'success'


                elif (command_exist.name == "USERINFO" or command_exist.name == 'CHECK'
                      or command_exist.name == "ATTLOG" or command_exist.name == "REBOOT"
                      or command_exist.name == "ENROLL_FP"):
                    command_exist.status = 'success'
