        ...
        if r == True and self.picking_type_id.code == 'incoming':
            ...
            if orden_compra:
                ...
                for producto in productos_orden:
                    ...
                    cantidad_comprada_hecha = sum(self.move_line_ids_without_package.filtered(lambda y: y.product_id == producto).mapped('quantity'))
                    on_hand_qty_dic.update({producto.default_code:qty_on_hand - cantidad_comprada_hecha})
                    done_qtys_dic.update({producto.default_code: sum(self.move_line_ids_without_package.filtered(lambda y: y.product_id == producto).mapped('quantity'))})
