...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        # MIGRACIÓN V19: `move_line_ids_without_package` fue eliminado ->
        # `move_line_ids`; `qty_done` -> `quantity` (con `picked` para
        # replicar la semántica de "cantidad ya hecha").
        old_price_dic = {x.default_code: x.product_tmpl_id.standard_price for x in self.move_line_ids.product_id}
        ...
        if r == True and self.picking_type_id.code == 'incoming':
            ...
            if orden_compra:
                ...
                for producto in productos_orden:
                    ...
                    cantidad_comprada_hecha = sum(
                        self.move_line_ids.filtered(lambda y: y.product_id == producto and y.picked).mapped('quantity')
                    )
                    on_hand_qty_dic.update({producto.default_code: qty_on_hand - cantidad_comprada_hecha})
                    done_qtys_dic.update({producto.default_code: cantidad_comprada_hecha})
